...
「它在前台」是按**前台窗口属于哪个进程**判断的，所以浏览器的会话恢复提示这类
辅助窗口挡在前面时也算数，不会出现「按一下没反应」。

这两步都可以按需改掉——动作类型选「窗口切换」时，编辑器里有**两个开关**：

| 开关 | 关掉之后 |
|---|---|
| 已打开时回到原来那个窗口 | 每次点都**新开一个窗口**，已经开着的保持不动 |
| 已经在前台时收起窗口 | 已经在前台时只把它**重新顶到最前**，不收起 |
